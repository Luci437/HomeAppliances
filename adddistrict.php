...
if (isset($_POST['add-district'])) {
    ...
    $subcategoryname = $_POST['state-id'];
    $category_select = $_POST['district-name'];
  

     $sql4="select * from district where district_name='$category_select'";
    ...
    if($row8==0){
        ...
        $sql1="INSERT INTO `district`( `district_name`,`state_id`,`status`) VALUES ('$category_select',$subcategoryname,1)";
        
        $res = mysqli_query($con, $sql1)or die(mysqli_error($con));


        $p = "select max(district_id) as lgid from district";
     ...
     echo '<script> alert("District Already Existed! ")</script>';
}
    }
...
<form method="post" action="" style="text-align: center;">
            <select class="input-box" name="state-id" id="category_select">
            ...
            <input type="text" class="input-box" name="district-name">
            <input type="submit" name="add-district" value="Add" class="submit-button" placeholder="Country name here">
        </form>
        ...
        <div class="available-countries">
          ...
          while($row = mysqli_fetch_assoc($result)) {
            echo '<h5>'.$row["district_name"].' <a href="delete-district.php?did='.$row["district_id"].'" class="delete-button">X</a></h5>';
          }
          ?>
        </div>
